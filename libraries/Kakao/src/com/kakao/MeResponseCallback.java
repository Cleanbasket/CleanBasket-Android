...
/**
 * 사용자정보 요청 ({@link UserManagement#requestMe(com.kakao.MeResponseCallback)}) 호출할 때 넘겨주고 콜백을 받는다.
 * @author MJ
 */
public abstract class MeResponseCallback extends UserProfileResponseCallback {
     ...
     * 가입이 안된 경우라면 {@link #onNotSignedUp()}을 호출하고, 그외의 경우 세션의 상태에 따라 {@link #onSessionClosedFailure(APIErrorResult)} 콜백 또는 {@link com.kakao.MeResponseCallback#onFailure(APIErrorResult)}} 콜백을 호출한다.
    ...
    /**
     * 에러가 발생했을 때의 처리로
     * 세션이 닫혔으면 {@link #onSessionClosedFailure(APIErrorResult)} 콜백을 아니면 {@link com.kakao.MeResponseCallback#onFailure(APIErrorResult)}} 콜백을 호출한다.
     * @param msg 로깅 메시지
